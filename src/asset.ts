...
export class asset {
  ...
  async getGalleries(guid: string) {
    ...
    const initialRecords = await apiClient.assetMethods.getGalleries(guid, '', pageSize, rowIndex);

    const totalRecords = initialRecords.totalCount;
    ...
    if (multiExport) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        index += 1;
        const galleries = await apiClient.assetMethods.getGalleries(guid, '', pageSize, rowIndex);
  ...
  async getAssets(guid: string) {
    ...
    const initialRecords = await apiClient.assetMethods.getMediaList(pageSize, recordOffset, guid);
      ...
      if (folderPath) {
        ...
        try {
          await fileExport.downloadFile(
            originUrl,
            `.agility-files/assets/${folderPath}/${fileName}`
          );
        } catch {
          this.unProcessedAssets[assetMediaID] = fileName;
        }
    ...
    if (multiExport) {
      for (let i = 0; i < iterations; i++) {
        ...
        const assets = await apiClient.assetMethods.getMediaList(pageSize, recordOffset, guid);
