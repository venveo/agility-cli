...
export class push {
  ...
  getContentModelsFromLocalStore(guid: string, locale: string, isPreview: boolean) {
    let fileOperation = new fileOperations();
  ...
  getContentListsFromLocalStore(guid: string, locale: string, isPreview: boolean) {
    let fileOperation = new fileOperations();
  ...
  async getPageModelsFromLocalStore(guid?: string, locale?: string, isPreview?: boolean) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        let pageTemplate = JSON.parse(files[i]) as mgmtApi.PageModel;
  ...
  async getPagesFromLocalStore(guid?: string, locale?: string, isPreview?: boolean) {
    let fileOperation = new fileOperations();

    let files = fileOperation.readDirectory(`${guid}/${locale}/${isPreview ? "preview" : "live"}/pages`);
  ...
  async createBaseContentItems(guid: string, locale: string, isPreview: boolean, targetGuid?: string) {
    ...
    if (fileOperation.folderExists(`${guid}/${locale}/${isPreview ? "preview" : "live"}/item`)) {
      ...
      validBar1.update(0, { name: "Content List Items: Validation" });
  ...
  async pushTemplates(
    ...
    for (let i = 0; i < templates.length; i++) {
      ...
      try {
        let createdTemplate = await apiClient.pageMethods.savePageTemplate(targetGuid, locale, template);
  ...
  async pushPages(guid: string, locale: string, pages: mgmtApi.PageItem[]) {
    ...
    //   if(parentPages[i].name === 'home' || parentPages[i].name === 'fire') {
        await this.processPageOld(parentPages[i], guid, locale, false);
    //   }
    }
  ...
  async processPage(page: mgmtApi.PageItem, guid: string, locale: string, isChildPage: boolean) {
    ...
    //   }
      ...
      if (page) {
        ...
        console.log(ansiColors.yellow("Creating page"), page.name, "channelID:", page.channelID, "pageID:", page.pageID);
        console.log(page)
        try {
        let createdPage = await apiClient.pageMethods.savePage(page, guid, locale, parentPageID, -1);
        ...
        if (createdPage[0]) {
          if (createdPage[0] > 0) {
            ...
            console.log("3. Unable to create page for name >", page.name, "< with pageID", oldPageId);
            fileOperation.appendLogFile(`\n Unable to create page for name ${page.name} with pageID ${oldPageId}.`);
          }
        }
        } catch(err) {
  ...
  async pushNormalContentItems(guid: string, locale: string, contentItems: mgmtApi.ContentItem[], targetGuid?: string) {
    let apiClient = new mgmtApi.ApiClient(this._options);
    let fileOperation = new fileOperations();
    const progressBar6 = this._multibar.create(contentItems.length, 0);
    progressBar6.update(0, { name: "Content List Items: Non Linked" });
    ...
    for (let i = 0; i < contentItems.length; i++) {
          ...
          if (typeof fieldVal === "object") {
            if (Array.isArray(fieldVal)) {
              for (let k = 0; k < fieldVal.length; k++) {
                let retUrl = await this.changeOriginKey(targetGuid, fieldVal[k].url);
                contentItem.fields[fieldName][k].url = retUrl;
              }
            } else {
              if ("url" in fieldVal) {
                let retUrl = await this.changeOriginKey(targetGuid, fieldVal.url);
      ...
      let createdContentItemId = await apiClient.contentMethods.saveContentItem(contentItem, targetGuid, locale);
  ...
  async pushLinkedContentItems(guid: string, locale: string, contentItems: mgmtApi.ContentItem[], targetGuid?: string) {
    let apiClient = new mgmtApi.ApiClient(this._options);
    let fileOperation = new fileOperations();
    const progressBar7 = this._multibar.create(contentItems.length, 0);
    progressBar7.update(0, { name: "Content List Items: Linked" });
              ...
              if (field.type === "Content") {
                        ...
                        if (this.processedContentIds[id]) {
                          ...
                          try {
                            let file = fileOperation.readFile(
                              `.agility-files/${guid}/${locale}/${isPreview ? "preview" : "live"}/item/${id}.json`
                            );
                            contentItem = null;
                            break;
                          } catch {
                ...
                if ("contentid" in fieldVal) {
                  ...
                  if (this.processedContentIds[linkedContentId]) {
                    let file = fileOperation.readFile(
                      `.agility-files/${guid}/${locale}/${isPreview ? "preview" : "live"}/item/${linkedContentId}.json`
                    );
                ...
                if ("referencename" in fieldVal) {
                  ...
                  try {
                    let container = await apiClient.containerMethods.getContainerByReferenceName(refName, targetGuid);
                    if (!container) {
                      this.skippedContentItems[contentItem.contentID] = contentItem.properties.referenceName;
                      fileOperation.appendLogFile(
                        `\n Unable to find a container for content item referenceName ${contentItem.properties.referenceName} with contentId ${contentItem.contentID}.`
                ...
                if ("sortids" in fieldVal) {
                  ...
                  for (let s = 0; s < sortids.length; s++) {
                    ...
                    if (this.processedContentIds[sortid]) {
                      ...
                      try {
                        let file = fileOperation.readFile(
                          `.agility-files/${guid}/${locale}/${isPreview ? "preview" : "live"}/item/${sortid}.json`
                        );
                        contentItem = null;
                        break;
                      } catch {
  ...
  async pushNormalModels(model: mgmtApi.Model, guid: string, targetGuid: string) {
    let procesedModel = await this.createModel(model, guid, targetGuid);
    // console.log('Processed Model', procesedModel);
    return procesedModel;
  }

  async pushContainers(containers: mgmtApi.Container[], models: mgmtApi.Model[], guid: string, targetGuid?: string) {
    let apiClient = new mgmtApi.ApiClient(this._options);
    try {
      const progressBar5 = this._multibar.create(containers.length, 0);
      progressBar5.update(0, { name: "Content Lists" });
        ...
        if (referenceName) {
          ...
          if (modelID) {
            ...
            try {
              ...
              await apiClient.containerMethods.saveContainer(container, targetGuid);
            } catch {
              container.contentViewID = -1;
              await apiClient.containerMethods.saveContainer(container, targetGuid);
            }
  ...
  async pushLinkedModels(models: mgmtApi.Model[], guid: string, targetGuid?: string) {
    let apiClient = new mgmtApi.ApiClient(this._options);
    let fileOperation = new fileOperations();
    ...
    progressBar4.update(0, { name: "Content Models: Linked" });
    let index = 1;
    do {
      for (let i = 0; i < models.length; i++) {
        ...
        try {
          let existing = await apiClient.modelMethods.getModelByReferenceName(model.referenceName, targetGuid);
          if (existing) {
            let updatesToModel = this.updateModel(existing, model);
            updatesToModel.id = existing.id;
            let updatedModel = await apiClient.modelMethods.saveModel(updatesToModel, targetGuid);
            ...
            if (field.settings["ContentDefinition"]) {
              ...
              if (model.referenceName !== modelRef) {
                if (this.processedModels[modelRef] && !this.processedModels[model.referenceName]) {
                  model.id = 0;
                  try {
                    let createdModel = await apiClient.modelMethods.saveModel(model, targetGuid);
  ...
  async createModel(model: mgmtApi.Model, guid: string, targetGuid: string) {
    let apiClient = new mgmtApi.ApiClient(this._options);
    try {
      let existing = await apiClient.modelMethods.getModelByReferenceName(model.referenceName, targetGuid);
      ...
      if (existing) {
        ...
        model.id = 0;
        let newModel = await apiClient.modelMethods.saveModel(model, targetGuid);
  ...
  async pushPageModelsFromLocalStore(guid: string, targetGuid: string, locale: string, isPreview: boolean) {
    let pageModels = await this.getPageModelsFromLocalStore(guid, locale, isPreview);
    if (pageModels) {
      await this.pushTemplates(pageModels, guid, locale, isPreview, targetGuid);
    }
  }

  async pushPagesFromLocalStore(guid: string, targetGuid: string, locale: string, isPreview: boolean) {
    let pages = await this.getPagesFromLocalStore(guid, locale, isPreview);
    if (pages) {
      await this.pushPages(targetGuid, locale, pages);
    }
  }

  async pushContentModelsFromLocalStore(guid: string, targetGuid: string, locale: string, isPreview: boolean) {
    let contentModels = this.getContentModelsFromLocalStore(guid, locale, isPreview);
    if (contentModels) {
  ...
  async pushInstance(guid: string, targetGuid: string, locale: string, isPreview: boolean) {
    // TODO: we want to check if the target instance is empty
    // if not, we should offer to clean the instance

    console.log(`Pushing Instance ${guid} to ${targetGuid}`);
    console.log('\n');

    await this.pushPageModelsFromLocalStore(guid, targetGuid, locale, isPreview);
   
    try {
      let fileOperation = new fileOperations();
      fileOperation.createLogFile("logs", "instancelog");

      let models = this.getContentModelsFromLocalStore(guid, locale, isPreview);

      if (models) {
        let containers = this.getContentListsFromLocalStore(guid, locale, isPreview);
        ...
        progressBar3.update(0, { name: "Content Models: Non Linked" });

        // push pages
        await this.pushPagesFromLocalStore(guid, targetGuid, locale, isPreview);

        // push normal content
        let index = 1;
        for (let i = 0; i < normalModels.length; i++) {
          let normalModel = normalModels[i];
          await this.pushNormalModels(normalModel, guid, targetGuid);
          progressBar3.update(index);
          index += 1;
        }
        
        // push linked content
        await this.pushLinkedModels(linkedModels, guid, targetGuid);
        let containerModels = this.createBaseModels(null, guid, locale, isPreview);
        if (containers) {
          await this.pushContainers(containers, containerModels, guid, targetGuid);
          let contentItems = await this.createBaseContentItems(guid, locale, isPreview, targetGuid);
          if (contentItems) {
            ...
            await this.pushNormalContentItems(guid, locale, normalContentItems, targetGuid);
            await this.pushLinkedContentItems(guid, locale, linkedContentItems, targetGuid);
          }
        }
        ...
        // this._multibar.stop();
        console.log('\n');
        console.log("Instance Pushed Successfully");
        console.log('\n');
        homePrompt();
      } else {
        fileOperation.appendLogFile(
