...
export class StatusCommand extends BaseCommand {
  async execute(): Promise<void> {
    ...
    console.log(
      colors.yellow('🔐 Authentication:'),
      hasAuth ? colors.green('✅ Logged in') : colors.red('❌ Not logged in')
    );

    // Check stored instance GUID
    const storedGuid = this.getStoredInstanceGuid();
    console.log(
      colors.yellow('🏢 Stored Instance:'),
      storedGuid ? colors.green(`✅ ${storedGuid}`) : colors.gray('❌ None stored')
    );

    // Check for .agility-files folder
    const hasAgilityFiles = this.context.fileOps.cliFolderExists();
    console.log(
      colors.yellow('📁 Local Data:'),
      hasAgilityFiles ? colors.green('✅ .agility-files exists') : colors.gray('❌ No local data')
    );
    ...
    if (hasAgilityFiles) {
      ...
      try {
        ...
        console.log(
          colors.yellow('📦 Containers:'),
          colors.green(`✅ ${containerFiles.length} found`)
        );
      } catch {
    ...
    if (!hasAuth) {
      ...
      console.log(colors.yellow('💡 Next steps:'));
      console.log(
        colors.gray('  1. Run `agility pull --locale en-us --channel website` to download data')
      );
      ...
      console.log(
        colors.gray('  • `agility pull --locale en-us --channel website` - Refresh local data')
      );
      console.log(
        colors.gray('  • `agility generate-types` - Generate TypeScript types and Zod schemas')
      );
